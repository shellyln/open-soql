...
describe("pub-sub-1", function() {
    it("Pub / Sub (1)", async function() {
        for (const cf of resolverConfigs) {
            ...
            const { soql, insert, update, remove, touch, subscribe, unsubscribe } = build({
                resolvers: {
                    query: {
            ...
            const fnSubContactWildcard2: Subscriber = (param) => {
                callbackResults.push(param);
            };
            ...
            subscribe('Contact', 'Contact/z2', fnSubContactWildcard2);
    ...
    it("Pub / Sub (1); + transaction", async function() {
        for (const cf of resolverConfigs) {
            setDefaultStaticResolverConfig(cf);
            ...
            await transaction(async ({soql, insert, update, remove, touch}) => {
                const testData = await soql`
                    Select Foo, Bar, Baz, Qux, Quux, Corge, Grault, Garply, Count
                    from Contact`;
